( function() {
	...
	$( input ).change( function() {
		...
		var templateDimensions = $( '#template option:selected' ).data( 'dimensions' );
		if( templateDimensions[0] < $( window).width() ) {
	...
	//	create object to define crop tool settings 
	function prepareCrop() {
		...
		//	restrict crop tool to chosen dimensions & aspect ratio
	...
	function displayUploadedImage( cropOptions ) {
		...
		//	disable upload button, enable delete button
	...
	//	send details of crop area to server to make changes to image
	$( '#crop' ).click( function() {

		//	disable delete and crop buttons as a precaution
		$( '#delete' ).attr( 'disabled', 'disabled' );
		$( '#crop' ).attr( 'disabled', 'disabled' );
		$( '#template' ).attr( 'disabled', 'disabled' );

		//	if there has been no template selected ( indicated by selectedTemplateIndex being 0), dimensions = false
		//	if there has been a template selected, dimensions = an array of the selected template dimensions
		...
		function( res ) {
			...
			//	parse response from server, remove jcrop from image, reset image css 
			//	update image source to display newly cropped image 
			//	update newly cropped image details as data attributes
